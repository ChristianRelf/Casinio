  ...
  try {
    ...
    if (route.startsWith("bot/")) {
      ...
      if (route === "bot/tables" && request.method === "POST") {
        const input = z.object({ discordUserId: z.string().min(3), name: z.string().min(2).max(40), minBet: z.number().int().optional(), maxBet: z.number().int().optional(), maxSeats: z.number().int().optional() }).parse(await bodyJson(request));
      ...
      if (botClose && request.method === "POST") { const table = await getD1().prepare("SELECT owner_user_id FROM tables WHERE id=?").bind(botClose[1]).first<{ owner_user_id: string }>(); if (!table) throw new HttpError(404, "TABLE_NOT_FOUND", "Table not found"); return apiOk(await closeTable(botClose[1], table.owner_user_id), requestId); }
    ...
    if (route === "tables/join" && request.method === "POST") return idempotentMutation(request, requestId, session.user.id, route, async () => { const input = z.object({ code: z.string().min(4).max(20) }).parse(await bodyJson(request)); return joinWithInvite(session.user.id, input.code); });

    const inviteValidation = route.match(/^invites\/([^/]+)$/);
    if (inviteValidation && request.method === "GET") return apiOk(await validateInvite(inviteValidation[1]), requestId);
    ...
    if (tableId && actionRoute === "stream" && request.method === "GET") {
      const events = await getEvents(tableId, session.user.id, Number(url.searchParams.get("since") ?? request.headers.get("last-event-id") ?? 0));
      const stream = events.map((item) => `id: ${item.version}\nevent: table-event\ndata: ${JSON.stringify(item)}\n\n`).join("") || `event: heartbeat\ndata: ${JSON.stringify({ tableId, timestamp: new Date().toISOString() })}\n\n`;
      return new Response(`retry: 800\n${stream}`, { headers: { "content-type": "text/event-stream", "cache-control": "no-cache, no-store", connection: "keep-alive", "x-accel-buffering": "no" } });
    }
