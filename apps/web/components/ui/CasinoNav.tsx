...
function AnimatedBalance({ value }: { value: number }) {
  ...
  useEffect(() => {
    ...
    if (reduce) { current.current = value; const frame = requestAnimationFrame(() => setDisplay(value)); return () => cancelAnimationFrame(frame); }
...
export function CasinoNav({ user, tableName, inviteCode }: { user?: SessionUser | null; tableName?: string; inviteCode?: string | null }) {
  ...
  return <header className="casino-nav">
    ...
    <div className="nav-account"><span className="play-money-note">PLAY MONEY ONLY</span>{user && <><Link href="/profile" className="balance-badge"><span className="coin-dot">$</span><AnimatedBalance value={user.balance} /></Link><button className="avatar-button" onClick={() => router.push("/settings")} aria-label="Open settings"><Initials name={user.displayName} /></button><button className="quiet-button nav-signout" onClick={async () => { await api("auth/logout", { method: "POST", body: "{}", idempotent: false }); router.replace("/"); }}>Sign out</button></>}</div>
  </header>;
}
