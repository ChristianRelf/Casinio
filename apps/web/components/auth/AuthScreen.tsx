...
export function AuthScreen() {
  const router = useRouter(); const [confirmed, setConfirmed] = useState(false); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [devEnabled,setDevEnabled]=useState(false);
  useEffect(()=>{ api<{user:SessionUser}|null>("auth/session").then((session)=>{if(session)router.replace("/lobby")}).catch(()=>undefined);api<{enabled:boolean}>("auth/dev/status").then((status)=>setDevEnabled(status.enabled)).catch(()=>setDevEnabled(false)); },[router]);
  const devLogin = async (persona: "chris"|"maya"|"arthur") => { if(!confirmed){setError("Confirm the age and play-money notice first.");return} setBusy(true);setError("");try{await api("auth/dev",{method:"POST",body:JSON.stringify({persona,ageConfirmed:true}),idempotent:false});router.push("/lobby");router.refresh()}catch(reason){setError(reason instanceof Error?reason.message:"Sign-in failed")}finally{setBusy(false)} };
  return <main className="auth-screen">
    ...
    <section className="signin-card" aria-labelledby="signin-title"><span className="overline">WELCOME TO THE TABLE</span><h2 id="signin-title">One sensible confirmation.</h2><p>Low Stakes is a social game. Its currency has no cash value and cannot be bought, sold, withdrawn, or won as a prize.</p>
      ...
      {devEnabled&&<div className="dev-signin"><span>DEVELOPMENT BYPASS</span><p>Test identities only. Discord is not linked.</p><div><button disabled={busy} onClick={()=>devLogin("chris")}>Enter as Chris</button><button disabled={busy} onClick={()=>devLogin("maya")}>Enter as Maya</button><button disabled={busy} onClick={()=>devLogin("arthur")}>Enter as Arthur</button></div></div>}
